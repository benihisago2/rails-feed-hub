...
RSpec.describe FeedHttpClient do
  ...
  describe "#call" do
    it "returns the response body as a UTF-8 string" do
      stub_request(:get, feed.url).to_return(status: 200, body: "<rss></rss>")

      body = client.call(feed)
    ...
    it "raises FeedFetcher::TimeoutError when the request times out" do
      ...
      expect { client.call(feed) }.to raise_error(FeedFetcher::TimeoutError, /timed out/)
    end

    it "raises FeedFetcher::HttpError carrying the status on a non-2xx response" do
      stub_request(:get, feed.url).to_return(status: 404, body: "Not Found")

      expect { client.call(feed) }.to raise_error(FeedFetcher::HttpError) { |error|
